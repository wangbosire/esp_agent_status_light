...
flowchart LR
    ...
    User --> BleCmd["esp ble config / scan / test"]

    Event --> Command
    Command --> HookInput["HookInput<br/>raw_input + input_json"]
    HookInput --> LogPort
    ...
    Daemon --> Idle["idle_shutdown_loop<br/>1 小时无事件自动停机"]
    ...
    BleCmd --> BleConfig["ble.json<br/>设备名 / UUID"]
...
| IPC 客户端/服务端 | `IpcTransport`       | `UnixSocketTransport` / `UnixSocketServer`、`NamedPipeTransport` / `NamedPipeServer` | 平台差异、调试降级方案                  |
| 灯设备         | `LightDevice`        | `BtleplugBleAdapter`、`MockLightDevice`                              | BLE 库替换、测试、未来 USB/串口设备       |
| Runtime 存储  | `RuntimeStore`       | `FsRuntimeAdapter`、测试 fake runtime                                | 文件系统路径、测试隔离、权限差异             |
| 日志          | `EventLog`           | `JsonlLogAdapter`、测试 fake log                                     | 日志格式、测试、后续 GUI 展示            |
...
| 时间       | `chrono`                         | 日志时间、状态过期                                 |
| 日志       | JSONL 文件                        | `events.log` 面向用户排障，`runtime.log` 面向链路排障    |
...
esp stop [--force]
esp ble config [--name <NAME>] [--service-uuid <UUID>] [--mode-char-uuid <UUID>] [--reset]
esp ble scan [--duration <SECONDS>]
esp ble test [--mode <MODE>]
...
- 单元测试使用 mock / fake adapter 实现同一组 trait，验证核心逻辑不依赖真实 BLE、真实 IPC 或真实文件系统。
...
| 测试 fake runtime | 单元测试和集成测试中按需实现 `RuntimeStore` |
...
- `runtime/events.log` 使用 JSON Lines 记录用户可见事件，`esp logs` 读取它。
- `runtime/runtime.log` 使用 JSON Lines 记录运行链路细节，并定期裁剪到最近约 3000 条。
- `runtime/*.lock` 是跨进程串行化锁，owner 写入 `pid + token + start_signature` 的 JSON，兼容旧版本 pid-only 锁文件。
- 文件锁 Drop 只删除 token 匹配的锁，避免旧 owner 误删后继 owner；当平台无法读取进程启动签名时，stale 判断会保守地信任活跃 PID。
...
| 平台            | IPC                  |
| ------------- | -------------------- |
| macOS / Linux | `UnixSocketTransport` / `UnixSocketServer` |
| Windows       | `NamedPipeTransport` / `NamedPipeServer`   |

当前仓库未启用 TCP fallback。若后续增加调试用 loopback adapter，应只允许显式调试参数启用，并且监听 `127.0.0.1` 随机端口，不写入默认安装配置。Hook 场景中本机 socket/pipe 的边界更清晰，默认只推荐 Unix Socket / Named Pipe。
...
同一个 `source + session` 内当前采用“最新状态优先”的策略，并只保留一个针对 AI 生成态的体验保护：

| 当前状态 | 新事件 | 处理 |
| --- | --- | --- |
| 当前状态已过期 | 任意新事件 | 直接接管 |
| 当前为 `ai` | 新事件为 `busy`，`capability=RunningCommand`，且 `semantics=Continuation` | 保留当前 `ai`，避免泛化 continuation 把内容生成态冲掉 |
| 任意未过期状态 | `candidate.updated_at >= current.updated_at` | 用新状态覆盖当前状态 |
| 任意未过期状态 | `candidate.updated_at < current.updated_at` | 忽略更旧状态 |

全局 `effective mode` 的优先级比较只发生在不同 `source/session` 之间；单个会话自己的状态快照必须反映该会话最新上报的 mode。因此 `Stop`、`SubagentStop`、`sessionEnd` 等生命周期事件可以覆盖同 session 中较早的 `error` / `alarm`，这与当前测试里的“同会话最新状态优先”一致。
...
用户显式传入 `--ttl` 时，会优先使用命令行值；当前实现会拒绝超过 `chrono::Duration` 可表达范围的超大秒数，避免溢出。
...
`esp send` 进入 daemon 后，router 接受状态更新即返回 `accepted`，BLE 写入作为后台副作用排队执行。这样 Hook 不会因为蓝牙扫描、连接、写入或超时而阻塞 Agent 主流程。

后台 `sync_effective_mode()` 的当前规则：

1. 先读取一次 `initial_effective`，用于日志和同步上下文。
2. 获取 `LightDevice` mutex，执行 `health()`。
3. 如果设备断开，先 `connect()`；如果连接长时间空闲，写前主动刷新连接。
4. 真正写入 BLE 前，再从 router 重新读取最新 `effective mode`。
5. 只有最新 effective 与 `last_applied_mode` 不同，或本轮发生重连 / `force_write=true` 时才调用 `write_mode()`。
6. 写入成功后更新 `last_applied_mode`、`last_ble_write_at` 和 `DeviceHealth`。

第 4 步是并发安全边界：旧同步任务即使已经在 device lock、health 或 reconnect 上排队，也不能拿着过期 mode 覆盖后来的新状态。

后台定时任务每 1 秒清理过期状态，清理后如果 effective mode 发生变化，会触发同一套后台同步逻辑。

### 8.6 daemon 空闲自动停止

daemon 会维护最近一次成功接收并处理 `send` / Hook 事件的时间。若连续 1 小时没有任何事件触发，后台 idle watchdog 会发送 shutdown 信号，让 daemon 走正常收尾路径：灭灯、断开 BLE、清理 pid / IPC 信息和启动锁。

该自动停止只影响后台进程常驻时间，不改变 Hook 调用方式。下一次 `esp send` 发现 IPC 不可用时，会继续通过 `request_with_auto_start()` 自动拉起 daemon，并重试原始 IPC 请求。
...
BLE 设备信息保存在 `<runtime_root>/ble.json`，包括：

```json
{
  "device_name": "AgentStatusLight",
  "service_uuid": "b8b7e001-7a6b-4f4f-9a8b-11c0ffee0001",
  "mode_char_uuid": "b8b7e002-7a6b-4f4f-9a8b-11c0ffee0001"
}
```

`esp ble config` 负责读写该配置；`esp ble scan` 使用当前配置标记 `matches_config`；`esp ble test` 独立创建 `BtleplugBleAdapter` 连接设备，可选写入一个测试 mode。daemon 启动时也读取同一份配置，保证配置、扫描、测试和正式写入使用同一目标设备信息。

daemon 启动后执行：

1. 初始化 `LightDevice` adapter。
2. 写入 runtime pid / IPC 信息，使 daemon 尽快进入可响应状态。
3. 后台尝试扫描广播名为 `AgentStatusLight` 或包含目标 Service UUID 的设备。
4. 连接设备。
5. 发现 GATT 服务和 characteristic。
6. 后续在 `send`、TTL 过期切换、重连补写时，把最新 effective mode 写入设备。

如果多个设备同时出现，第一阶段选择 RSSI 最强的设备，并在日志中记录 device id。后续可增加 `esp pair` 或按 peripheral id 固定具体设备。
当前已支持通过 `esp ble config` 指定设备名、Service UUID 和 mode characteristic UUID；无需修改代码即可适配重命名或自定义 UUID 的固件。

首次 BLE 连接不会阻塞 daemon ready：即使设备暂时不可用，IPC `status` 和 `send` 仍应可用，设备恢复后由后台重连和同步链路补写最新状态。

### 9.3 重连策略

- BLE 断开后立即标记 `ble=disconnected`。
- 采用指数退避重连：1s、2s、5s、10s、30s，最大 30s。
- 重连成功后通过 `sync_effective_mode(true)` 强制补写当前最新 effective mode，即使 mode 没变化也会重新写一次。
- 如果最近一次成功写入已经超过空闲刷新阈值，下一次写前会主动 reconnect，降低陈旧 BLE 连接导致首次写入卡住的概率。
...
- daemon 已接受状态后，BLE 写入失败只更新日志和 `DeviceHealth`，不会回滚 router 状态，也不会反向让本次 IPC `send` 失败。
...
trait EventLog {
    ...
    fn append_runtime(&self, event: LogEvent) -> Result<()>;
...
`events.log` 不记录完整 Hook stdin，只保留面向用户的事件事实。`runtime.log` 用于链路排障，会在 `send.stdin_loaded` 记录 Hook 原始输入与解析状态：

- `raw_input`：宿主工具传入的原始 stdin 文本。
- `input_json`：解析后的 JSON；解析失败时为 `{}`，维持旧的 fallback 行为。
- `input_parse_error`：JSON 解析失败时的错误文本。
- `input_timed_out`：stdin 读取是否超时。

由于 `raw_input` 可能包含项目路径、工具参数或提示上下文，分享 `runtime.log` 前需要先脱敏。

`JsonlLogAdapter::tail()` 读取 `events.log` 时会跳过损坏 JSONL 行，并继续返回其它可解析日志。这样即使日志文件中出现半写、手动编辑或历史坏行，`esp logs` 仍能用于排障。

`events.log` 记录用户可见事实，例如 daemon 启停、`send` 状态更新被接受等；`runtime.log` 比 `events.log` 更细，包含 `send.received`、`send.stdin_loaded`、`send.mode_resolved`、`ipc.send_completed`、`ble.sync_started`、`ble.sync_effective_refreshed`、`daemon_boot.pid_stale`、`daemon.idle_shutdown` 等链路阶段。它会按写入次数低频裁剪，避免长期运行后无限增长。
...
- 当前仓库未启用 TCP IPC；若后续增加 TCP loopback，只能显式调试开启，正式 Hook 安装不得写入 TCP IPC 配置。
...
- 状态池更新规则：同 session 最新状态优先，且泛化 `busy` continuation 不冲掉当前 `ai` 生成态。
...
- daemon 前台启动。
- 使用测试 fake runtime / fake log / `MockLightDevice` 跑通 daemon。
- IPC send/status/stop 全流程，当前覆盖 Unix Socket、Named Pipe。
- 当前覆盖 Unix Socket、Named Pipe；若后续加入 TCP loopback，只允许在显式调试开关下使用，默认安装和默认运行不启用。
- daemon 不存在时 `esp send` 自动拉起。
- `LightDevice` 不可用时 quiet/strict 退出码差异。
- 多 source/session 上报时 effective mode 正确回落。
- 同 source/session 的最新状态会覆盖旧状态，除非候选是泛化 `busy` continuation 且当前是 `ai`。
- daemon 后台同步会在真正写 BLE 前刷新 latest effective，避免旧任务覆盖新状态。
- 文件锁 owner JSON 可兼容旧 pid-only 锁文件，Drop 不会误删后继 owner。
- `esp logs` 在日志文件含有坏行时仍能返回可解析日志。
...
| 未来 TCP loopback 无 token | 本机其它进程可控制灯             | 当前不启用 TCP IPC；若后续加入，只允许显式调试开启，正式安装只使用 socket/pipe |
| daemon 异常退出          | 状态灯不更新                 | `esp send` 自动拉起；pid / lock owner 健康检查；日志提示                 |
| 多 Agent 状态互相覆盖       | 展示误导                   | source/session 状态池与优先级路由                                      |
| 后台同步任务写回旧 effective | 设备显示过期状态              | 写 BLE 前重新读取最新 effective；失败只更新日志与 health，不回滚 router 状态 |
| 日志文件存在坏行            | `esp logs` 完全失败          | `tail()` 跳过坏行并保留可解析记录                                       |
